...
//
// All output goes to stderr — never stdout — because in MCP server mode
// stdout is the JSON-RPC channel and any non-JSON byte there will crash
// the client. console.log writes to stdout; console.error writes to stderr.
// Routing every level through stderr keeps both MCP and CLI modes safe.
export const logger = {
  levels: { error: 0, warn: 1, info: 2, debug: 3 },
  currentLevel: 2,

  error: (message, ...args) => logger.currentLevel >= 0 && console.error(`❌ [ERROR] ${message}`, ...args),
  warn:  (message, ...args) => logger.currentLevel >= 1 && console.error(`⚠️ [WARN] ${message}`, ...args),
  info:  (message, ...args) => logger.currentLevel >= 2 && console.error(`ℹ️ [INFO] ${message}`, ...args),
  debug: (message, ...args) => logger.currentLevel >= 3 && console.error(`🐛 [DEBUG] ${message}`, ...args)
};
...
# Research-enhanced models
PERPLEXITY_API_KEY=your_perplexity_api_key_here

# API Configuration
# Options: claude, openai, perplexity
